    ...
    return (
        <section ref={ref} className={cn('scroll-section bg-background', inView ? 'is-visible' : 'opacity-0')}>
            ...
            <div className="relative z-10 w-full max-w-4xl mx-auto p-8 flex flex-col items-center text-center gap-8 animate-item" style={{ animationName: 'zoom-in' }}>
                <h2 className="font-headline text-3xl md:text-4xl title-underline pb-4">Solutions &amp; Synthesis</h2>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 my-4 w-full">
                      {solutions.map((solution, index) => (
                          <div key={index} className={cn(
                              "flex items-center gap-4 p-4 bg-secondary/10 backdrop-blur-sm rounded-lg border border-foreground/10 animate-item",
                ...
                <p className={cn("font-accent italic text-xl md:text-2xl mt-8 animate-item", inView && "gold-shimmer-text")} style={{ animationName: 'fade-up', animationDelay: '1.5s' }}>
