            ...
            <defs>
                <pattern id="neuralPattern" patternUnits="userSpaceOnUse" width="100" height="100">
                    <circle cx="10" cy="10" r="2" fill="hsl(var(--accent))">
                        {inView && <animate attributeName="r" values="2;4;2" dur="3s" repeatCount="indefinite" />}
                    </circle>
                    <circle cx="50" cy="70" r="3" fill="hsl(var(--accent))">
                        {inView && <animate attributeName="r" values="3;5;3" dur="4s" begin="1s" repeatCount="indefinite" />}
                    </circle>
                    <circle cx="90" cy="30" r="2" fill="hsl(var(--accent))">
                         {inView && <animate attributeName="r" values="2;4;2" dur="3.5s" begin="2s" repeatCount="indefinite" />}
                    </circle>
    ...
    return (
        <section ref={ref} className={cn('scroll-section bg-background', inView ? 'is-visible' : 'opacity-0')}>
            ...
            <div className="relative z-10 w-full max-w-7xl mx-auto p-8 grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
                ...
                <div className="flex flex-col gap-4 font-code text-sm md:text-base bg-black/40 backdrop-blur-sm p-6 rounded-lg border border-accent/50 animate-item shadow-lg" style={{ animationName: 'fade-up', animationDelay: '0.4s' }}>
